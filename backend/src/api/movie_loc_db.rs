...
use crate::{handle_field::{get_content_type, get_name, create_file, get_filename, extract_text}, models::NewMovieFileLoation};
...
async fn upload_episodes(mut payload: Multipart, conn: Data<MysqlConnection>) -> Json<String> {
        ...
        match get_content_type(&field).as_str() {
            "video/mp4" => {
                if get_name(&field) == "file" {
                    ...
                    match create_file(field, format!("{}/{}", &filepath.as_ref().unwrap(), &filename.as_ref().unwrap())).await {
                        Ok(_) => (),
    ...
    if fileupload && matches!(&epi, Some(_value)) && matches!(&movie, Some(_value))  && matches!(&filename, Some(_value)) { 
        let testing = super::super::models::MovieFileLoationService{conn: &conn};
        //TODO: Delete file on error db error
        match testing.add(NewMovieFileLoation {
            uuid,
            movie: movie.unwrap(),
            epi: epi.unwrap(),
            name,
